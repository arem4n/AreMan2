...
const caseStudyComponents: { [key: string]: React.ComponentType<any> } = {
  'areman-escudo-heraldico': Arem4nCaseStudy,
  'arem4n-professional-brand': Arem4nProfessionalCaseStudy,
  'osttech-cybersecurity': OstTechCaseStudy,
  'albornoz-propiedades': AlbornozCaseStudy,
  'southsoft-development': SouthSoftCaseStudy,
  'bm3-constructora': Bm3CaseStudy,
  'tommybox-training': TommyBoxCaseStudy,
};


const renderCaseStudy = (slug: string) => {
    const Component = caseStudyComponents[slug] || GenericCaseStudy;
    return <Component slug={slug} />;
};


const LogoCodexClient: React.FC<{ slug?: string }> = ({ slug }) => {
    const router = useRouter();
    const pathname = usePathname();

    const selectedSlug = slug || 'areman-escudo-heraldico';

    const handleSelectSlug = (newSlug: string) => {
        trackEvent('select_case_study', { slug: newSlug });
        router.push(`/logocodex/${newSlug}`, { scroll: true });
    };
                    ...
                    <div className="max-w-6xl mx-auto px-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-6">
                            {portfolioProjects.map(project => (
                                <button
                                    key={project.slug}
                                    onClick={() => handleSelectSlug(project.slug)}
                                    className={`block text-left p-3 rounded-xl transition-all duration-300 ${selectedSlug === project.slug ? 'bg-symbolic-500/20 ring-2 ring-symbolic-500' : 'bg-white hover:bg-deep-50 shadow-md hover:shadow-lg'}`}
                                >
                                    <div className="aspect-square w-full rounded-lg overflow-hidden mb-3">
                                      <img src={project.imageUrl} alt={project.title} className="w-full h-full object-cover" />
                                    </div>
                                    <h3 className="font-bold text-sm text-deep-800">{project.title}</h3>
                                    <p className="text-xs text-deep-600">{project.category}</p>
                                </button>
                            ))}
                        </div>
