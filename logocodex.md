...
> I’m ready to help you build interfaces that feel right. My knowledge combines AREM4N’s brand design system with Emil Kowalski’s design engineering philosophy. If you want to dive even deeper into the engineering side, check out Emil’s course: [animations.dev](https://animations.dev/).
...
## Brand Context: AREM4N Design System

### Users

Dual audience: **tech founders** in early/growth stage who want to differentiate and justify premium prices, AND **traditional SMEs** scaling up who need to modernize their image to compete. Both arrive looking for strategic authority — someone who understands the real problem behind a logo, not just an aesthetic service. The ideal first impression is "finalmente alguien que lo entiende."

### Brand Personality

**Estratégico · Preciso · Austero**

AREM4N is a one-person studio by Sergio Arellano (Puerto Montt, Chile) with a cinematic background applied to brand strategy. The tone is confrontational and direct (the copy proves it: "tu competencia tiene un logo parecido al tuyo"). The methodology — LogoCodeX™ — positions this as a system, not decoration. The brand should feel like a high-conviction consultant, not a creative freelancer.

### Aesthetic Direction

- **Theme**: Dark sections are appropriate (founders and SME decision-makers often browse in focused environments), but the site alternates between dark hero and light sections — maintain that rhythm.
- **Palette constraint**: pink/symbolic (`#ec4899` family / `symbolic-*` tokens), deep slate (`#0f172a` family / `deep-*` tokens), and creative yellow (`#eab308` family / `creative-*` tokens) are established brand colors — keep the palette base but refine execution (eliminate gradients, use solids with intention).
- **Anti-references**: Generic agency sites, gradient-heavy portfolios, glassmorphism UI kits, "AI slop" dark mode with glowing accents.
- **References**: Editorial design, high-conviction consulting firms, luxury brand agencies — where the design IS the proof of capability.

### Design Principles

1. **Austero no es vacío** — Every element must earn its place. No decorative gradients, no floating animations for aesthetic effect. Visual restraint IS the message.
2. **La marca es la prueba** — AREM4N itself is the first LogoCodeX™ case study. The site must demonstrate the methodology’s rigor, not contradict it with generic AI patterns.
3. **Copy-forward** — The writing is the strongest asset. Design should amplify the words, not compete with them. Typography and whitespace over visual noise.
4. **Precisión tipográfica** — Fonts must be distinctive and intentional. Inter + Playfair Display are reflex choices that undercut the premium positioning.
5. **Conversión sobre expresión** — The site is in production and serving real clients. Every design decision should reduce friction toward the "Solicitar Auditoría" CTA.
